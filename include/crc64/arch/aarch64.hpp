...
namespace crc64 {
    ...
    namespace detail {
        ...
        static inline SIMD from_mul(poly64_t a, poly64_t b) {
            auto mul = vmull_p64(a, b);
            return SIMD(vreinterpretq_u8_p128(mul));
        }

        static inline Poly64Pair into_poly64pair(SIMD x) {
            auto y = vreinterpretq_p64_u8(x._inner);
            return {vgetq_lane_p64(y, 0), vgetq_lane_p64(y, 1)};
        }

        static inline poly64_t high64(SIMD x) {
            auto y = vreinterpretq_p64_u8(x._inner);
            return vgetq_lane_p64(y, 1);
        }

        static inline poly64_t low64(SIMD x) {
            auto y = vreinterpretq_p64_u8(x._inner);
    ...
    inline uint64_t SIMD::barrett(uint64_t poly, uint64_t mu) const noexcept {
        auto t1 = detail::low64(
                detail::from_mul(detail::low64(*this), static_cast<poly64_t>(mu)));
        auto l  = detail::from_mul(t1, static_cast<poly64_t>(mu));
        auto reduced = static_cast<uint64_t>(detail::high64(bitxor(l)));
